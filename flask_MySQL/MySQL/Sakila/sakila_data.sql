...
SELECT actor.actor_id, film.title, actor.first_name, film.description, film.release_year
FROM film
...
WHERE store.store_id IN(1,42,312,459);
...
-- 8. What query would you run to get all the action films which are joined by SANDRA KILMER? 
-- Your query should return film title, description, release year, rating, special features, genre (category), and actor's first name and last name.
SELECT actor.actor_id, film.title, actor.first_name, film.description, film.release_year, actor.first_name, actor.last_name
FROM film
    JOIN film_actor ON film.film_id = film_actor.film_id
    JOIN actor ON film_actor.actor_id = actor.actor_id
    JOIN film_category ON film.film_id = film_category.film_id
...
-- answer for 8 on a different way
